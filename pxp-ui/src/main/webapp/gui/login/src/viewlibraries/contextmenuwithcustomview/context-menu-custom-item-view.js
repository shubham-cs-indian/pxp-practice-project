  ...
  getHtmlContextMenuItemList = (aModelList) => {
    ...
    CS.forEach(aModelList, function (oModel, iIndex) {
      ...
      if (React.isValidElement(oModel.properties.view)) {
        ...
        if (oModel.icon) {
          oIconColorDom = that.getImageView(oModel.icon)
        }
        ...
        if (oModel.properties.code) {
          var oModelCode = <div className='contextMenuItemCode'>{oModel.properties.code}</div>
        }
        ...
        aMenuItemList.push(
            <div className={sContextMenuItemClass}
              ...
              {oCheckbox}
              {oIconColorDom}
              <div className="contextMenuData">{CS.getLabelOrCode(oModel)}</div>
              {oModelCode}
              {oSelectedIconDOM}
            </div>
        );
