...
public class QuickListSupplierInstanceForRelationshipController extends BaseController
    implements IRuntimeController {
  
  @Autowired
  IQuickListSupplierInstanceForRelationship quickListSupplierInstanceRelationship;
  
  @RequestMapping(value = "/supplierinstance/relationship/quicklist", method = RequestMethod.POST)
  public IRESTModel execute(@RequestBody RelationshipInstanceQuickListModel quickListModel)
      throws Exception
  {
    return createResponse(quickListSupplierInstanceRelationship.execute(quickListModel));
  }
}