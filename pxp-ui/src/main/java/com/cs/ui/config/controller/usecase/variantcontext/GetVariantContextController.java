...
public class GetVariantContextController extends BaseController implements IConfigController {
  
  @Autowired
  IGetVariantContext getVariantContext;
  
  @RequestMapping(value = "/variantcontexts/{id}", method = RequestMethod.GET)
  public IRESTModel execute(@PathVariable String id) throws Exception
  {
    IIdParameterModel idModel = new IdParameterModel(id);
    return createResponse(getVariantContext.execute(idModel));
  }
}
