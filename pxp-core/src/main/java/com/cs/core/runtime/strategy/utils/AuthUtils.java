...
public class AuthUtils {
  ...
  public static boolean check(String password, String stored) throws Exception
  {
    ...
    if (saltAndPass.length != 2) {
      // throw new IllegalStateException("The stored password has the form
      // 'salt$hash'");
      RDBMSLogger.instance().info("\n\nThe stored password does not have the encrypted form!!!");
      return password.equals(stored);
    }
    String hashOfInput = hash(password, Base64.getDecoder()
