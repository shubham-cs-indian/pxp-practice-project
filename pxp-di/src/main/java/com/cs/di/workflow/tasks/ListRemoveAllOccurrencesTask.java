...
@Component("listRemoveAllOccurrencesTask")
public class ListRemoveAllOccurrencesTask extends AbstractTask {
  ...
  public void executeTask(WorkflowTaskModel model)
  {
    ...
    Boolean output = modifiedList.removeAll(Collections.singleton(removeItem));
    if (!output) {
    ...
    model.getExecutionStatusTable().addSuccess(new ObjectCode[] { ObjectCode.CAMUNDA_TASK, ObjectCode.CAMUNDA_TASK_ID }, MessageCode.GEN035,
        new String[] { ListRemoveAllOccurrencesTask.class.getName(), model.getTaskId() });
