...
@Component
public abstract class AbstractSaveInstanceForTabs<P extends IModel, R extends IModel> extends AbstractRuntimeService<P, R> {
  ...
  @Autowired
  protected SaveInstanceValidations                     saveInstanceValidations;
  ...
  protected R executeInternal(P instancesModel) throws Exception
  {
    ...
    saveInstanceValidations.validate(klassInstancesModel);
    ...
    List<IBaseEntityDTO> baseEntitiesByIIDs = rdbmsComponentUtils.getLocaleCatlogDAO(languageInheritance)
        .getBaseEntitiesByIIDs(List.of(klassInstancesModel.getId()));
    IBaseEntityDAO baseEntityDAO = this.rdbmsComponentUtils.getBaseEntityDAO(baseEntitiesByIIDs.get(0));

    IMulticlassificationRequestModel multiClassificationRequestModel = this.configUtil
        .getConfigRequestModelForSaveInstance(klassInstancesModel.getTemplateId(), klassInstancesModel.getTabId(), klassInstancesModel.getTypeId(),
    ...
    rdbmsComponentUtils.getLocaleCatlogDAO().postUsecaseUpdate(Long.parseLong(klassInstancesModel.getId()),
   ...
   * @return
   */
  protected boolean shouldCreateRevision(IKlassInstanceSaveModel klassInstancesModel, IGetConfigDetailsForCustomTabModel configDetails)
  {
    List<String> versionableAttributes = configDetails.getVersionableAttributes();
  ...
  public void prepareDataAndCreateImageVariants(IGetConfigDetailsForCustomTabModel configDetails,
      IImageAttributeInstance coverFlowAttribute, String instanceId, String parentId, IGetKlassInstanceCustomTabModel returnModel)
      throws Exception
  {
      ...
      IGetConfigDetailsForCustomTabModel configDetails) throws Exception
  {
    ...
    // Below Method is used for LanguageInheritance Purpose only
    // Method checks if added attributes already exist in Postgress
    checkIfAddedAttributesAlreadyContainsInDatabaseForLanguageInheritance(klassInstancesModel, baseEntityDAO);


    this.manageProperties(klassInstancesModel, configDetails, baseEntityDAO);

    RuleHandler ruleHandler = new RuleHandler();
    IChangedPropertiesDTO changedProperties = ruleHandler.initiateRuleHandlingForSave(baseEntityDAO, rdbmsComponentUtils.getLocaleCatlogDAO(), false,
        configDetails.getReferencedElements(), configDetails.getReferencedAttributes(), configDetails.getReferencedTags());

    changedProperties.getAttributeCodes().forEach(attrCode -> {
      IModifiedContentAttributeInstanceModel modAttr = new ModifiedAttributeInstanceModel();
      modAttr.setAttributeId(attrCode);
      klassInstancesModel.getModifiedAttributes().add(modAttr);
    });

    changedProperties.getTagCodes().forEach(tagCode -> {
      IModifiedContentTagInstanceModel modTag = new ModifiedTagInstanceModel();
      modTag.setTagId(tagCode);
      klassInstancesModel.getModifiedTags().add(modTag);
    });
  }

  private void checkIfAddedAttributesAlreadyContainsInDatabaseForLanguageInheritance(IKlassInstanceSaveModel klassInstancesModel,
      IBaseEntityDAO baseEntityDAO) throws RDBMSException, CSFormatException
  {
    List<IContentAttributeInstance> addedAttributes = klassInstancesModel.getAddedAttributes();
    List<IModifiedContentAttributeInstanceModel> modifiedAttributes = klassInstancesModel.getModifiedAttributes();
    
    if(addedAttributes != null && !addedAttributes.isEmpty()) {
      
      ConfigurationDAO configurationDAO = ConfigurationDAO.instance();
      List<IPropertyDTO> propertyDTOs = new ArrayList<>();
      
      Map<String, IContentAttributeInstance> toBeRemovedFromAddedAttributes = new HashMap<>();
      for(IContentAttributeInstance addedAttribute : addedAttributes) {
        
        IPropertyDTO propertyDTO = configurationDAO.getPropertyByCode(addedAttribute.getAttributeId());
        propertyDTOs.add(propertyDTO);
        toBeRemovedFromAddedAttributes.put(addedAttribute.getAttributeId(), addedAttribute);
      }

      long baseEntityIID = baseEntityDAO.getBaseEntityDTO().getBaseEntityIID();
      Map<Long,Set<IPropertyRecordDTO>> propertyRecordsForEntities = rdbmsComponentUtils.getLocaleCatlogDAO().getPropertyRecordsForEntities(Set.of(baseEntityIID), propertyDTOs.toArray(new IPropertyDTO[0]));
      Set<IPropertyRecordDTO> propertyRecords = propertyRecordsForEntities.computeIfAbsent(baseEntityIID, k -> new HashSet<>());

      for(IPropertyRecordDTO recordDTO : propertyRecords) {
        if(((IValueRecordDTO)recordDTO).getContextualObject() == null) {  
          IContentAttributeInstance contentAttributeInstance = toBeRemovedFromAddedAttributes.get(recordDTO.getProperty().getPropertyCode());
          addedAttributes.remove(contentAttributeInstance);
        
          IModifiedContentAttributeInstanceModel modifiedAttribute = new ModifiedAttributeInstanceModel();
          modifiedAttribute.setKlassInstanceId(Long.toString(baseEntityDAO.getBaseEntityDTO().getBaseEntityIID()));
          modifiedAttribute.setBaseType(contentAttributeInstance.getBaseType());
          modifiedAttribute.setId(KlassInstanceBuilder.getAttributeID((IValueRecordDTO)recordDTO));
          modifiedAttribute.setAttributeId(contentAttributeInstance.getAttributeId());
          ((IModifiedAttributeInstanceModel)modifiedAttribute).setValue(((IAttributeInstance)contentAttributeInstance).getValue());
          ((IModifiedAttributeInstanceModel)modifiedAttribute).setValueAsHtml(((IAttributeInstance)contentAttributeInstance).getValueAsHtml());
          ((IModifiedAttributeInstanceModel)modifiedAttribute).setLanguage(((IAttributeInstance)contentAttributeInstance).getLanguage());
  ...
  protected void manageProperties(IKlassInstanceSaveModel klassInstancesModel, IGetConfigDetailsModel configDetails,
      ...
      IBaseEntityDAO baseEntityDAO) throws Exception
  {
    ...
    List<IPropertyRecordDTO> propertyRecordDTO = manageAddedAttributes(klassInstancesModel, configDetails, propertyRecordBuilder,
        baseEntityDAO);
      ...
      IGetConfigDetailsModel configDetails, PropertyRecordBuilder propertyRecordBuilder, IBaseEntityDAO baseEntityDAO) throws Exception
  {
    ...
    for (IModifiedContentAttributeInstanceModel modifiedAttribute : modifiedAttributes) {
      if (!modifiedAttribute.getAttributeId().equals(Constants.ASSET_COVERFLOW_ATTRIBUTE_ID)) {
        IPropertyRecordDTO updateValueRecord = propertyRecordBuilder.updateValueRecord(modifiedAttribute);
        if ((((IModifiedAttributeInstanceModel) modifiedAttribute).getModifiedContext() != null)
  ...
  {
    if (klassInstancesModel.getModifiedTags().isEmpty())
      return new ArrayList<IPropertyRecordDTO>();
    
    List<IModifiedContentTagInstanceModel> modifiedTags = klassInstancesModel.getModifiedTags();
    
    List<IPropertyRecordDTO> propertyRecordDTO = modifiedTags.stream()
        .filter(x -> ((ITagInstance) x.getEntity()).getContextInstanceId() == null).map(modifiedTag -> {
          try {
            if (configDetails.getReferencedTags().get(modifiedTag.getTagId()).getTagType()
                .equals(SystemLevelIds.LIFECYCLE_STATUS_TAG_TYPE_ID)) {
              propertyRecordBuilder.validateLifeStatusTag(modifiedTag);
            }
        ...
        }).filter(propertyRecord -> propertyRecord != null).collect(Collectors.toList());
  ...
  protected List<IPropertyRecordDTO> manageAddedTags(IKlassInstanceSaveModel klassInstancesModel, IGetConfigDetailsModel configDetails,
      PropertyRecordBuilder propertyRecordBuilder) throws Exception
  {
    if (klassInstancesModel.getAddedTags().isEmpty())
      return new ArrayList<IPropertyRecordDTO>();
    List<ITagInstance> addedTags = klassInstancesModel.getAddedTags();
    List<IPropertyRecordDTO> propertyRecordDTOList = addedTags.stream().filter(x -> x.getContextInstanceId() == null).map(addedTag -> {
      try {
      ...
      IBaseEntityDAO baseEntityDAO) throws Exception
  {
    ...
    List<Long> addedInstanceIds = klassInstancesModel.getAddedLinkedInstances().stream().map(x -> Long.parseLong(x.getId()))
        .collect(Collectors.toList());
    List<Long> deletedInstanceIds = klassInstancesModel.getDeletedLinkedInstances().stream().map(x -> Long.parseLong(x))
        .collect(Collectors.toList());
    ...
    if (shouldUpdateEntity) {
      String contextCode = contextualDataDTO.getContext().getContextCode();
      contextualDataDTO.setChanged(true);
      contextualDataDTO.setLinkedBaseEntityIIDs(linkedBaseEntityIIDs.toArray(new Long[linkedBaseEntityIIDs.size()]));
      IReferencedVariantContextModel referencedContext = configDetails.getReferencedVariantContexts().getEmbeddedVariantContexts()
          .get(contextCode);
      variantInstanceUtils.checkDuplicate(baseEntityDAO, baseEntityDTO.getParentIID(), referencedContext, true,
          baseEntityDTO.getParentIID());
