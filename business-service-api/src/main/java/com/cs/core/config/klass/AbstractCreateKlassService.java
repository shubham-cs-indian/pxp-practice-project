...
public abstract class AbstractCreateKlassService<P extends IKlassModel, R extends IGetKlassEntityWithoutKPModel>
  ...
  public R executeInternal(P klassModel) throws Exception
  {
    ...
    klassValidations.validate(klassModel);
    
    R executeCreateKlass = executeCreateKlass(klassModel);
    ITypesTaxonomiesModel typesTaxonomiesModel = new TypesTaxonomiesModel();
    typesTaxonomiesModel.getTypes().add(code);
    createDefaultInstanceService.execute(typesTaxonomiesModel);
    return executeCreateKlass;
  }
